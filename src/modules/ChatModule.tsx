...
// Quick-start prompts — African student exam prep as primary wedge
const QUICK_STARTS = [
    ...
    try {
        ...
        if (res.sources && res.sources.length > 0) {
          useGiaStore.setState(s => ({
            sessions: s.sessions.map(sess =>
              sess.id === sessionId
                ? { ...sess, messages: sess.messages.map(m => m.id === asstId ? { ...m, sources: res.sources, thinking: false } : m) }
                : sess
            ),
          }));
        }
      ...
      if (!ctrl.signal.aborted) {
        ...
        useGiaStore.setState(s => ({
          sessions: s.sessions.map(sess =>
            sess.id === sessionId
              ? { ...sess, messages: sess.messages.map(m => m.id === asstId ? { ...m, error: true, thinking: false } : m) }
              : sess
          ),
        }));
      }
    ...
    return (
      <div className="flex flex-col h-full" style={{ background: 'var(--gia-bg)' }}>
        <div className="flex items-center justify-between px-4 py-4 shrink-0" style={{ borderBottom: '1px solid var(--gia-border)' }}>
          ...
          <button
            onClick={() => { createSession(); setShowHistory(false); }}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {sessions.length === 0 && (
            <p className="text-xs text-center mt-8" style={{ color: 'var(--gia-muted)' }}>No chats yet.</p>
          )}
          {sessions.map((sess) => (
            <div
              key={sess.id}
              className="gia-card p-3 flex items-center gap-3 cursor-pointer transition-all tap-feedback"
              style={sess.id === activeSessionId ? { borderColor: 'rgba(168,85,247,0.3)', background: 'rgba(168,85,247,0.06)' } : {}}
              onClick={() => { setActiveSession(sess.id); setShowHistory(false); }}
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] mt-0.5" style={{ color: 'var(--gia-muted)' }}>
                  {sess.messages.length} msgs · {new Date(sess.updatedAt).toLocaleDateString()}
                </p>
              </div>
              <button
                ...
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--gia-muted-2)')}
              >
                <Trash2 size={13} />
              </button>
            </div>
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--gia-bg)' }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 shrink-0" style={{ borderBottom: '1px solid var(--gia-border)' }}>
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-lg transition-colors tap-feedback"
            style={{ color: 'var(--gia-muted)' }}
          >
            <History size={15} />
          </button>
          <span className="text-xs font-medium truncate max-w-[130px]" style={{ color: 'var(--gia-muted)' }}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={exportChat} className="p-1.5 rounded-lg tap-feedback" style={{ color: 'var(--gia-muted)' }}>
            <Download size={13} />
          </button>
          <button onClick={() => activeSessionId && clearSession(activeSessionId)} className="p-1.5 rounded-lg tap-feedback" style={{ color: 'var(--gia-muted)' }}>
            <Trash2 size={13} />
          </button>
          <button onClick={createSession} className="p-1.5 rounded-lg tap-feedback" style={{ color: 'var(--gia-muted)' }}>
            <Plus size={13} />
          </button>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center pb-16 animate-fade-in">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, rgba(168,85,247,0.2), rgba(124,58,237,0.1))', border: '1px solid rgba(168,85,247,0.2)' }}
            >
            ...
            <div>
              <p className="text-base font-semibold" style={{ color: 'var(--gia-text)' }}>
                {userProfile.name ? `Hey ${userProfile.name} ✦` : 'Hey, I\'m GIA'}
              </p>
              <p className="text-xs mt-1 max-w-[240px] leading-relaxed" style={{ color: 'var(--gia-muted)' }}>
                {providerConnected
                  ? 'Your personal AI workspace. Ask anything, attach files, or pick a quick start below.'
                  : 'Connect a provider in Settings → Engine Room to get started.'}
              </p>
            </div>
        ...
        {messages.map((msg, i) => (
          ...
          >
            ...
            <div
              ...
              style={{
                background: msg.role === 'user'
                  ...
                  : msg.error ? 'rgba(239,68,68,0.15)' : 'var(--gia-surface-2)',
                border: msg.role === 'assistant' ? '1px solid var(--gia-border)' : 'none',
              }}
            >
              {msg.role === 'user'
                ...
                : msg.error
                  ...
                  : msg.thinking
                    ? <div className="flex gap-0.5">{[0,1,2].map(d => <div key={d} className="thinking-dot" style={{ animationDelay: `${d * 0.16}s` }} />)}</div>
                    : <Bot size={13} style={{ color: 'var(--gia-muted)' }} />
              }
            </div>
            ...
            <div className="flex-1 min-w-0 space-y-1">
              ...
              {/* Bubble */}
              <div className={`inline-block max-w-full px-4 py-3 ${
                msg.role === 'user' ? 'msg-user text-sm' : msg.error ? 'msg-error w-full' : 'msg-assistant w-full'
              }`}>
                {msg.thinking
                  ? <div className="flex gap-1.5 py-0.5">{[0,1,2].map(d => <div key={d} className="thinking-dot" style={{ animationDelay: `${d * 0.16}s` }} />)}</div>
                ...
                {msg.attachments?.filter(a => !a.preview).map(att => (
                  <div key={att.name} className="mt-2 flex items-center gap-1.5 text-[10px] px-2 py-1.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.1)' }}>
                ...
                {msg.sources && msg.sources.length > 0 && (
                  <div className="mt-2 pt-2 space-y-1" style={{ borderTop: '1px solid var(--gia-border)' }}>
                ...
                {/* Model tag */}
                {msg.model && !msg.thinking && msg.role === 'assistant' && (
                  <div className="mt-1.5 text-[9px]" style={{ color: 'var(--gia-muted-2)' }}>via {msg.model}</div>
                )}
              </div>

              {/* Action row */}
              {msg.role === 'assistant' && !msg.thinking && msg.content && (
                <div className="flex gap-1 ml-1 opacity-0 group-hover:opacity-100 transition-all">
                  <button
                    onClick={() => copyMessage(msg.id, msg.content)}
                    className="text-[10px] flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors"
                    style={{ color: copiedId === msg.id ? '#34d399' : 'var(--gia-muted)' }}
                  >
                  ...
                  {messages.length > 4 && (
                    <button
                      onClick={() => { if (activeSessionId) forkSession(activeSessionId, i); }}
                      className="text-[10px] flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors"
                      style={{ color: 'var(--gia-muted)' }}
                    >
      ...
      <AnimatePresence>
        {showScrollBtn && (
          <motion.button
            ...
            className="absolute right-4 bottom-32 w-8 h-8 rounded-full flex items-center justify-center shadow-lg z-10"
            style={{ background: 'var(--gia-surface-3)', border: '1px solid var(--gia-border)' }}
          >
        ...
        <div className="mx-4 mb-2 flex gap-2 overflow-x-auto pb-1">
          {attachments.map((att, idx) => (
            <div key={idx} className="relative shrink-0 group">
              {att.preview
                ? <img src={att.preview} alt={att.name} className="w-14 h-14 rounded-xl object-cover" style={{ border: '1px solid rgba(168,85,247,0.2)' }} />
                : (
                  <div className="w-14 h-14 rounded-xl flex flex-col items-center justify-center gap-1" style={{ background: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.2)' }}>
                    <Paperclip size={12} style={{ color: '#a855f7' }} />
                    <span className="text-[8px] max-w-[50px] truncate text-center" style={{ color: '#a855f7' }}>{att.name}</span>
                  </div>
                )
              }
      ...
      <div className="px-4 pb-5 pt-1 shrink-0">
        <input ref={fileRef} type="file" className="hidden" multiple onChange={e => handleFile(e)}
          accept=".txt,.md,.pdf,.csv,.json,.js,.ts,.tsx,.py,.html,.css,.xml,.yaml" />
        ...
        <div className="flex items-center gap-1.5 mb-2.5 flex-wrap">
          {[
            ...
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2.5 py-1.5 rounded-xl border transition-all tap-feedback"
              style={{
                background: tool.active ? `${tool.activeColor}20` : 'var(--gia-surface)',
                border: `1px solid ${tool.active ? `${tool.activeColor}40` : 'var(--gia-border)'}`,
                color: tool.active ? tool.activeColor : 'var(--gia-muted)',
                fontWeight: 500,
              }}
            >
              <tool.icon size={11} />
        ...
        <AmbientInput
          ...
          isLoading={loading}
          placeholder={webSearch ? 'Ask anything — I\'ll search the web…' : 'Message GIA…'}
        />
      </div>
    </div>
  );
