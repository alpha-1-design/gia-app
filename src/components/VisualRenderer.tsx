...
const MapVisual: React.FC<{ data: any }> = ({ data }) => {
  const [copied, copy] = useCopy();
  ...
  const markers: { lat: number; lng: number; label?: string; color?: string }[] = data?.markers || [];
  const route: { lat: number; lng: number }[] = data?.route || [];
  ...
  }, [center.lat, center.lng, zoom, JSON.stringify(markers), JSON.stringify(route)]);
