...
class ConnectorManager {
  ...
  async testConnection(id: string): Promise<boolean> {
    ...
    try {
      ...
      const headers: Record<string, string> = { 'User-Agent': 'GIA/2.4.0.7' };
  ...
  async call(id: string, request: ConnectorRequest): Promise<ConnectorResponse> {
    ...
    let urlStr: string;
    const headers: Record<string, string> = { 'User-Agent': 'GIA/2.4.0.7', ...request.headers };
  ...
  async callRaw(url: string, method: string, headers?: Record<string, string>, body?: unknown): Promise<ConnectorResponse> {
    ...
    const res = await fetch(url, {
      method,
      headers: { 'User-Agent': 'GIA/2.4.0.7', ...headers },
