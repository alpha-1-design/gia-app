  ...
  execute: async () => {
    ...
    try {
      ...
      const lines = [
        ...
        `- **Disk Free (est.):** ${(() => { const infoAny = info as Record<string, unknown>; const v = infoAny.diskFree; return typeof v === 'number' ? `${(v / 1024 / 1024 / 1024).toFixed(1)} GB` : 'Unknown'; })()}`,
        `- **Disk Total (est.):** ${(() => { const infoAny = info as Record<string, unknown>; const v = infoAny.diskTotal; return typeof v === 'number' ? `${(v / 1024 / 1024 / 1024).toFixed(1)} GB` : 'Unknown'; })()}`,
        '',
        '**Power**',
