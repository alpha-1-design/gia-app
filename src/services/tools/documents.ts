...
/**
 * Mirror of server/browse_web.py, embedded so browse_web works in the
 * on-device sandbox too. Same self-provisioning pattern as READ_DOC_SCRIPT:
 * the server copy is never uploaded to /workspace, so a fresh native/remote
 * rootfs has nothing to run at the old hardcoded path.
 */
const BROWSE_WEB_SCRIPT = `#!/usr/bin/env python3
...
const browse_web: Tool = {
  ...
  execute: async (args) => {
    ...
    try {
      ...
      const scriptPath = '/workspace/browse_web.py';
      await SandboxService.writeFile(scriptPath, BROWSE_WEB_SCRIPT);

      let result;
      try {
        result = await SandboxService.exec(`python3 ${scriptPath} '${payload.replace(/'/g, "'\\''")}'`);
      } finally {
        // Never leave the helper lingering in the workspace — same cleanup
        // discipline as read_document's read_doc.py.
        await SandboxService.delete(scriptPath);
      }
