...
ALPINE_PKGS="bash curl wget python3 nodejs npm build-base git proot openssh sudo vim sqlite ca-certificates jq zip unzip rsync openssl"
...
build_and_export() {
  ...
  if [[ "$image" == alpine* ]]; then
    ...
    docker exec "$cid" sh -c "mkdir -p /root /workspace && echo 'nameserver 8.8.8.8
