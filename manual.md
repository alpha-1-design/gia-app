...
| Fix | Description |
|-----|-------------|
| **Offline Mode that actually works** | One toggle in Settings → Reliability blocks every tool that needs the network, and tells the model *which* tool was blocked and why — so GIA adapts its answer instead of retrying a call that will never be allowed. On-device tools (memory, notes, tasks, clipboard, local files, local ML) keep working. |
| **Real permission system** | Camera, location, contacts, SMS and friends are no longer denied silently by Android. GIA asks the first time a tool needs one, shows an in-app Permissions panel so you can see and change every grant, and reports a clear denial back to the model. |
| **GIA now knows its own interface** | Ask "where do I turn on X?" and `app_map` returns the real path (e.g. *Settings → System & Performance → Voice*) rather than an invented one. The old behaviour was confidently making up menu paths that didn't exist. |
| **Nexus sub-agents actually fire** | The sub-agent tool had a schema/provider mismatch that made delegation fail instantly for anyone not on OpenAI. It now defaults to your *active* provider and falls back to the first enabled one. |
| **Replies no longer freeze mid-stream** | Message rows are memoized, so streaming no longer re-renders every message in the conversation on each token. This was the other half of the "GIA stopped responding" reports. |
| **Local model recommendations** | GIA checks your device RAM and recommends a specific model at the chat entry point, with a warning that an oversized model is the usual cause of local models freezing part-way through an answer. |
...
| **Full Install — a linker error that broke every package** | Even with correct code, proot itself could not start: it was linked against `libtalloc.so.2` but the bundled allocator was stored as `libtalloc.so`. Android's linker matches dependency names exactly, so every proot launch died with `CANNOT LINK EXECUTABLE`. One mismatch, and it is why *every* package (python3, nodejs, git, …) failed at once. The bundled binary's `DT_NEEDED` entry is now corrected — see `android/app/src/main/jniLibs/THIRD_PARTY_LICENSES.md` for the exact patch, since it means our copy of that binary is no longer byte-identical to upstream. |
| **Chat no longer hangs when a provider stalls** | If a provider stopped sending data mid-answer — rate-limit backoff, a wedged proxy, half-open connection — nothing timed out and the reply froze indefinitely. There's now a guard that watches for silence: 30s between chunks, 60s for the very first byte (a slow start or a long reasoning block is normal and is not treated as a hang). A long answer that's *still* arriving is never cut off. |
...
| **Mind maps are actually visible** | The agent could generate a perfect mind map and the card would hide it: the root node was drawn at x=0 and clipped off the left edge, zoom was applied twice, "collapse" still left an empty box, and the grab cursor was connected to no handler at all. The map now fits your screen, scrolls in both directions, drags to pan, and collapses to a single line. |
| **Smart home controls work on a phone** | Smart-home discovery and control were pointed at the desktop companion server, which doesn't exist on a phone — so they always failed. They now fall back to the on-device terminal like the rest of GIA's tools. |
